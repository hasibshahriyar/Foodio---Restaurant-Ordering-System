...
export default function AdminOrdersPage() {
  ...
  return (
    <>
      {openDropdown && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 15 }} onClick={() => setOpenDropdown(null)} />
      )}
                    ...
                    <div style={{ position: 'relative', display: 'inline-block' }}>
                      ...
                      {openDropdown === order.id && (
                        <div
                          ...
                          style={{ position: 'absolute', top: '100%', left: 0, marginTop: 4, width: 130, background: 'white', border: '1px solid #E6E2D8', borderRadius: 8, zIndex: 20, overflow: 'hidden' }}
                        >
                          {STATUS_OPTIONS.map((s) => (
                            <button
                              key={s}
                              onClick={() => handleStatusChange(order.id, s)}
                              style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 12px', fontFamily: 'Manrope, sans-serif', fontSize: 13, color: '#1A1A1A', background: order.status === s ? '#F2EFE9' : 'transparent', border: 'none', cursor: 'pointer', textAlign: 'left' }}
                            >
                              {s}
                              {order.status === s && (
