...
export default function EditItemModal({ item, onClose, onSuccess }: Props) {
  ...
  const currentImageName = item.image ? item.image.split('/').pop() : null;
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 50, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
      <div style={{ width: 512, background: '#FBFAF8', border: '1px solid #E6E2D8', borderRadius: 12, display: 'flex', flexDirection: 'column', maxHeight: '90vh', overflow: 'hidden' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 24px', borderBottom: '1px solid #E6E2D8', flexShrink: 0 }}>
          <span style={{ fontFamily: 'Manrope, sans-serif', fontWeight: 700, fontSize: 18, color: '#1A3C34' }}>Edit Item</span>
        ...
        <form onSubmit={handleSubmit} style={{ padding: 24, display: 'flex', flexDirection: 'column', gap: 16, overflowY: 'auto' }}>
          ...
          <div>
            ...
            <input type="text" value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} required style={inputStyle} />
          </div>
          ...
          <div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <div
                ...
                style={{ flex: 1, height: 36, background: 'white', border: '1px solid #E6E2D8', borderRadius: 6, display: 'flex', alignItems: 'center', paddingLeft: 12, cursor: 'pointer', overflow: 'hidden' }}
              >
                <span style={{ fontFamily: 'Manrope, sans-serif', fontSize: 13, color: imageFile ? '#1A1A1A' : '#7A7A7A', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                  {imageFile ? imageFile.name : (currentImageName || 'Choose file...')}
                </span>
              </div>
              {imageFile && (
